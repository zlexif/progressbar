...
local function unpackRGBA(color)
    local num
    if #color == 6 then
        num = tonumber(color .. "FF", 16)
    else
        num = tonumber(color, 16)
    end
    local r = (num >> 24) & 0xFF
    local g = (num >> 16) & 0xFF
    local b = (num >> 8) & 0xFF
...
local function StartProgress(action, onStart, onTick, onFinish)
    ...
    if (not isPlayerDead or action.useWhileDead) and not isDoingAction then
        ...
        Action = action
        local r, g, b, a = unpackRGBA(cfg.color)
        SendNUIMessage({
            action = 'setColor',
            r = r,
            g = g,
...
ProgressBar.Progress = function(action, finish)
    StartProgress(action, nil, nil, finish)
end

ProgressBar.ProgressWithStartEvent = function(action, start, finish)
    StartProgress(action, start, nil, finish)
end

ProgressBar.ProgressWithTickEvent = function(action, tick, finish)
    StartProgress(action, nil, tick, finish)
end

ProgressBar.ProgressWithStartAndTick = function(action, start, tick, finish)
    StartProgress(action, start, tick, finish)
end

ProgressBar.isDoingSomething = function()
